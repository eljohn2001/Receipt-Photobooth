...
import { PaymentMethodView } from './views/payment-method';
...
if (isAdminRoute) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const views: Record<AppState, BaseView> = {
      ...
      'payment-method': new PaymentMethodView(document.getElementById('view-payment-method')!, navigateTo, session),
      'cash-instruction': new CashInstructionView(document.getElementById('view-cash-instruction')!, navigateTo, session),
      'qr-scanner': new QrScannerView(document.getElementById('view-qr-scanner')!, navigateTo, session),
      'payment': new PaymentView(document.getElementById('view-payment')!, navigateTo, session),
    ...
    function performNavigation(nextState: AppState, params?: any, forceInstant?: boolean) {
      ...
      if (nextState === 'idle') {
        ...
        session.selectedPackage = undefined;
        session.isPaid = false;
        session.isPackageLocked = false;
        session.paymentMethod = undefined;
        session.scannedPackageQr = undefined;
      }
    ...
    let currentPaymentQrDataUrl: string | null = null;
    let pendingBgFile: File | null = null;
    let bgFileType: 'image' | 'video' | null = null;
    let tempPackages: PrintPackage[] = [];
    let packageQrDataUrls: Record<string, string | null> = {};

    function updatePackageQrPreview(pkgId: string, dataUrl: string | null) {
      const container = document.getElementById(`preview-pkg-qr-container-${pkgId}`);
      if (!container) return;
      if (dataUrl) {
        container.innerHTML = `
          <img src="${dataUrl}" alt="Package QR Preview" style="max-height: 90px; border-radius: 4px; object-fit: contain; margin-top: 6px; display: block;" />
          <button type="button" class="btn-remove-media btn-remove-pkg-qr" data-pkg-id="${pkgId}" style="width: 100%; margin-top: 4px; font-size: 11px; padding: 4px 8px;">Remove QR</button>
        `;
    ...
    function updatePaymentQrPreview(dataUrl: string | null) {
      const container = document.getElementById('preview-payment-qr-container');
      if (!container) return;
      if (dataUrl) {
        container.innerHTML = `
        <img src="${dataUrl}" alt="Payment QR Preview" style="max-height: 120px; border-radius: 4px;" />
        <button type="button" class="btn-remove-media" id="btn-remove-payment-qr" style="width: 100%;">Remove</button>
      `;
        container.classList.remove('hidden');
      } else {
        container.innerHTML = '';
    ...
    async function populateAdminForm(config: KioskConfig) {
      ...
      const enablePaywallInput = document.getElementById('input-enable-paywall') as HTMLInputElement;
      if (enablePaywallInput) enablePaywallInput.checked = config.enablePaywall === true;

      const paymentAccountNameInput = document.getElementById('input-payment-account-name') as HTMLInputElement;
      if (paymentAccountNameInput) paymentAccountNameInput.value = config.paymentAccountName || '';

      const paymentAccountNumberInput = document.getElementById('input-payment-account-number') as HTMLInputElement;
      if (paymentAccountNumberInput) paymentAccountNumberInput.value = config.paymentAccountNumber || '';

      const paymentInstructionsInput = document.getElementById('input-payment-instructions') as HTMLTextAreaElement;
      if (paymentInstructionsInput) paymentInstructionsInput.value = config.paymentInstructions || '';

      currentPaymentQrDataUrl = config.paymentQrCodeDataUrl || null;
      updatePaymentQrPreview(currentPaymentQrDataUrl);
      ...
      // Populate package cards in Paywall tab
      const packageIds = ['pkg-a', 'pkg-b', 'pkg-c', 'pkg-d'];
      packageQrDataUrls = {};
      packageIds.forEach((id, idx) => {
        const pkg = (config.packages || []).find(p => p.id === id) || {
          id,
          name: `Package ${String.fromCharCode(65 + idx)}`,
          printsCount: idx + 1,
          price: idx === 0 ? 50 : idx === 1 ? 99 : idx === 2 ? 149 : 199,
          isEnabled: true,
          qrCodeDataUrl: null,
          inclusions: `${idx + 1} Thermal Print${idx > 0 ? 's' : ''} + Digital Softcopy`
        };

        packageQrDataUrls[id] = pkg.qrCodeDataUrl || null;

        const enabledCheck = document.getElementById(`input-pkg-enabled-${id}`) as HTMLInputElement;
        const pkgNameInput = document.getElementById(`input-pkg-name-${id}`) as HTMLInputElement;
        const pkgPriceInput = document.getElementById(`input-pkg-price-${id}`) as HTMLInputElement;
        const pkgPrintsInput = document.getElementById(`input-pkg-prints-${id}`) as HTMLInputElement;
        const pkgInclusionsInput = document.getElementById(`input-pkg-inclusions-${id}`) as HTMLInputElement;

        if (enabledCheck) enabledCheck.checked = pkg.isEnabled !== false;
        if (pkgNameInput) pkgNameInput.value = pkg.name || `Package ${String.fromCharCode(65 + idx)}`;
        if (pkgPriceInput) pkgPriceInput.value = (pkg.price !== undefined ? pkg.price : 0).toString();
        if (pkgPrintsInput) pkgPrintsInput.value = (pkg.printsCount || idx + 1).toString();
        if (pkgInclusionsInput) pkgInclusionsInput.value = pkg.inclusions || `${idx + 1} Thermal Print${idx > 0 ? 's' : ''} + Digital Softcopy`;

        updatePackageQrPreview(id, packageQrDataUrls[id]);
      });
    ...
    const paymentQrFileInput = document.getElementById('input-payment-qr-file') as HTMLInputElement;

    paymentQrFileInput?.addEventListener('change', () => {
      const file = paymentQrFileInput.files?.[0];
      if (file) {
        const reader = new FileReader();
        reader.onload = (event) => {
          const result = event.target?.result as string;
          currentPaymentQrDataUrl = result;
          updatePaymentQrPreview(result);
        };
        reader.readAsDataURL(file);
      }
    });

    // Package-specific QR Code upload listeners
    ['pkg-a', 'pkg-b', 'pkg-c', 'pkg-d'].forEach((id) => {
      const fileInput = document.getElementById(`input-pkg-qr-file-${id}`) as HTMLInputElement;
      fileInput?.addEventListener('change', () => {
        ...
        if (file) {
          ...
          reader.onload = (event) => {
            const result = event.target?.result as string;
            packageQrDataUrls[id] = result;
            updatePackageQrPreview(id, result);
          };
          reader.readAsDataURL(file);
        }
      });
    ...
    // Event delegation for package QR removal
    document.getElementById('kiosk-packages-setup-list')?.addEventListener('click', (e) => {
      const target = e.target as HTMLElement;
      if (target.classList.contains('btn-remove-pkg-qr')) {
        const pkgId = target.getAttribute('data-pkg-id');
        if (pkgId) {
          packageQrDataUrls[pkgId] = null;
          updatePackageQrPreview(pkgId, null);
          const input = document.getElementById(`input-pkg-qr-file-${pkgId}`) as HTMLInputElement;
    ...
    document.getElementById('preview-payment-qr-container')?.addEventListener('click', (e) => {
      const target = e.target as HTMLElement;
      if (target.id === 'btn-remove-payment-qr') {
        currentPaymentQrDataUrl = null;
        updatePaymentQrPreview(null);
    ...
    configForm?.addEventListener('submit', async (e) => {
      ...
      const newConfig: KioskConfig = {
        ...
        enablePaywall: enablePaywallInput ? enablePaywallInput.checked : false,
        paymentQrCodeDataUrl: currentPaymentQrDataUrl,
        paymentAccountName: paymentAccountNameInput ? paymentAccountNameInput.value.trim() : '',
        paymentAccountNumber: paymentAccountNumberInput ? paymentAccountNumberInput.value.trim() : '',
        paymentInstructions: paymentInstructionsInput ? paymentInstructionsInput.value.trim() : '',
        ...
        packages: ['pkg-a', 'pkg-b', 'pkg-c', 'pkg-d'].map((id, idx) => {
          const enabledCheck = document.getElementById(`input-pkg-enabled-${id}`) as HTMLInputElement;
          const nameInput = document.getElementById(`input-pkg-name-${id}`) as HTMLInputElement;
          const priceInput = document.getElementById(`input-pkg-price-${id}`) as HTMLInputElement;
          const printsInput = document.getElementById(`input-pkg-prints-${id}`) as HTMLInputElement;
          const inclusionsInput = document.getElementById(`input-pkg-inclusions-${id}`) as HTMLInputElement;
          const existingPkg = (currentKioskConfig.packages || []).find(p => p.id === id);

          return {
            id,
            name: nameInput ? nameInput.value.trim() : `Package ${String.fromCharCode(65 + idx)}`,
            price: priceInput ? parseFloat(priceInput.value) || 0 : (idx === 0 ? 50 : idx === 1 ? 99 : idx === 2 ? 149 : 199),
            printsCount: printsInput ? parseInt(printsInput.value, 10) || (idx + 1) : (idx + 1),
            inclusions: inclusionsInput ? inclusionsInput.value.trim() : `${idx + 1} Thermal Print + Digital Softcopy`,
            isEnabled: enabledCheck ? enabledCheck.checked : true,
            qrCodeDataUrl: packageQrDataUrls[id] !== undefined ? packageQrDataUrls[id] : (existingPkg?.qrCodeDataUrl || null)
          };
        }),
